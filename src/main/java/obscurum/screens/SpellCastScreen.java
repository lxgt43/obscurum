    ...
    public void displayOutput(AsciiPanel terminal) {
        ...
        if (entries.size() > 0) {
            ...
            for (int i = 0; i < scrollList.getHeight(); i++) {
                ...
                String display;
                DisplayColour displayColour = spell.getSpellType() == Spell.REST_SPELL && player.getCombatCooldown() > 0
                        ? DisplayColour.RED
                        : DisplayColour.WHITE;
                terminal.write(entries.get(index).getText(), topLeft.x + 2, topLeft.y + i + 3, displayColour);
                ...
                terminal.write(display, topLeft.x + 2 + xOffset, topLeft.y + i + 3);
                xOffset += display.length();
                displayColour = spell.getCurrentCooldown() == 0
                        ? DisplayColour.WHITE
                        : DisplayColour.BRIGHT_RED;
