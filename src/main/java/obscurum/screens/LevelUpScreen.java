  ...
  public void displayOutput(AsciiPanel terminal) {
    ...
    if (entries.size() > 0) {
      for (int i = 0; i < scrollList.getHeight(); i++) {
        ...
        displayColour = levelUpCosts[index] > player.getAttributePoints() ?
            Display.RED : Display.WHITE;
        terminal.write(entries.get(index).getText() + " Cost:", topLeft.x + 2,
            topLeft.y + i + 3);
        terminal.write(Integer.toString(levelUpCosts[index]),
            topLeft.x + 2 + xOffset, topLeft.y + i + 3, displayColour);
      }
