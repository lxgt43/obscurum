...
public class CharacterScreen extends Screen {
    ...
    private void printEquipmentList(AsciiPanel terminal) {
        ...
        for (int i = 0; i < EQUIPMENT_SLOT_COUNT; i++) {
            ...
            int tlY = 3 + i % 3 * 5;
            DisplayColour borderColour = highlighted == i ? DisplayColour.GREEN : Display.FG_WINDOW_FRAME;
            ...
            if (!player.getEquipment().getEquipment(i).isOfType(
                ...
                DisplayColour quality = Display.QUALITY_COLOURS[displayItem.getQuality()];
    ...
    private void printPlayerDepiction(AsciiPanel terminal) {
        ...
        switch (highlighted) {
            case Equipment.HEAD:
                for (int i = 0; i < 2; i++) {
                    terminal.write(PLAYER_DEPICTION[i].substring(0, PLAYER_DEPICTION[i].length() - 1), playerTLX, playerTLY + i, DisplayColour.GREEN);
                }
                break;
            case Equipment.CHEST:
                terminal.write(PLAYER_DEPICTION[2].substring(0, PLAYER_DEPICTION[2].length() - 1), playerTLX, playerTLY + 2, DisplayColour.GREEN);
                for (int i = 3; i < 5; i++) {
                    terminal.write(PLAYER_DEPICTION[i].substring(2, 4), playerTLX + 2, playerTLY + i, DisplayColour.GREEN);
                }
                break;
            case Equipment.HANDS:
                for (int i = 3; i < 5; i++) {
                    terminal.write(PLAYER_DEPICTION[i].substring(0, 2), playerTLX, playerTLY + i, DisplayColour.GREEN);
                    terminal.write(PLAYER_DEPICTION[i].substring(5, 7), playerTLX + 5, playerTLY + i, DisplayColour.GREEN);
                }
                break;
            case Equipment.WEAPON:
                for (int i = 0; i < 5; i++) {
                    terminal.write(PLAYER_DEPICTION[i].substring(PLAYER_DEPICTION[i].length() - 2), playerTLX + PLAYER_DEPICTION[i].length() - 2 , playerTLY + i, DisplayColour.GREEN);
                }
                break;
            case Equipment.LEGS:
                for (int i = 5; i < 8; i++) {
                    terminal.write(PLAYER_DEPICTION[i], playerTLX, playerTLY + i, DisplayColour.GREEN);
                }
                break;
            default:
                terminal.write(PLAYER_DEPICTION[8], playerTLX, playerTLY + 8, DisplayColour.GREEN);
                break;
        }
    }
    ...
    private void printPlayerAttributes(AsciiPanel terminal) {
        ...
        for (int i = 0; i < Creature.NUM_OF_ATTRIBUTES; i++) {
            ...
            outputColour = attributes[i] > baseAttributes[i] ? Display.QUALITY_COLOURS[1] : DisplayColour.WHITE;
            output = i == Creature.DODGE_CHANCE ?
                    Integer.toString(attributes[i]) + "%" :
                    Integer.toString(attributes[i]);
            terminal.write(output, ATTRIBUTES_TL_X + 1 + outputLength, ATTRIBUTES_TL_Y + 4 + i, outputColour);
